...
def res_linear_activation_v2_noise(input_size, resize_output, num_filters, kernel_size, num_channels, num_classes, resize=True,
    ...
    x1 = tf.image.resize(x1, [int(x.shape[1]), int(x.shape[2])],
                    method=tf.image.ResizeMethod.BILINEAR)
    x1 = res_block_initial(x1, [num_filters[2]], 5, [1, 1], f"noise_blockwcccc", sym_padding=False)
    x1 = res_block_initial(x1, [num_filters[2]], 5, [1, 1], f"noise_blockccdfcec", sym_padding=False)
    x = tf.keras.layers.Concatenate(-1)([x1, x])
    x = res_block_initial(x, [num_filters[3]], 5, [1, 1], f"noise_blockceedfdrere", sym_padding=False)
    x = res_block_initial(x, [num_filters[3]], 5, [1, 1], f"noise_blockdererdfde", sym_padding=False)
    x = res_block_initial(x, [num_filters[3]*2], 3, [1, 1], f"noise_blockereredfdre", sym_padding=False)
    # decode
    ...
    #noise_updated = noise2 * 0.05
    x = tf.keras.layers.Concatenate(-1)([noise2, x])
    x = up_block(x, temp1, kernel_size=5, filters = num_filters[1], i =3, sym_padding=False)
    output = tf.image.resize(x, (resize_output[0], resize_output[1]),
                    method=tf.image.ResizeMethod.BILINEAR)
    output = res_block_initial(output, [num_filters[1]],5, [1, 1], f"chur", sym_padding=False)
    output = res_block_initial(output, [num_filters[1]], 3, [1, 1], "output_convbbb123456", sym_padding=False)
    output = res_block_initial(output, [num_filters[0]], 3, [1, 1], "output_convbbb12341t6", sym_padding=False)
    output = res_block_initial(output, [num_filters[0]], 3, [1, 1], "output_convbbb123451212", sym_padding=False)
    output = res_block_initial(output, [num_filters[0]], 3, [1, 1], "output_convbbb123467", sym_padding=False)
