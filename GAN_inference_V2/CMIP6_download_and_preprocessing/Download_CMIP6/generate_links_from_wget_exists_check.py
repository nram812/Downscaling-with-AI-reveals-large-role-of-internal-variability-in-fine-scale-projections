# DM edits
import time
import pandas as pd
import numpy as np
from tqdm import tqdm
import requests
from multiprocessing.dummy import Pool as ThreadPool
import urllib
import pathlib
import os
import sys
import threading
from pathlib import Path
import argparse
import swifter

sys.path.append(r'/nesi/nobackup/niwa00018/CMIP6_data/download-cmip6/get-data')
from file_downloader import FileDownloader
from scrape_links import Scrape_WGET

# Argument parser
parser = argparse.ArgumentParser(description='Download CMIP6 Data')
parser.add_argument('-v', '--variables', help='variables to download', required=False, default=['ta', 'hus', 'ua', 'va'])
parser.add_argument('-s', '--ssps', help='ssps to download', required=False, default='historical')
parser.add_argument('-o', '--output_path', help='output path for data', required=False, default='/nesi/nobackup/niwa03712/rampaln/CMIP6_archive')
parser.add_argument('-w', '--wget', help='wget file (sh)', required=True)
parser.add_argument('-w_var', '--wget_var', help='wget variable obtained from esgf', required=True)
parser.add_argument('-w_ssp', '--wget_ssp', help='wget ssp obtained from esgf', required=True)
parser.add_argument('-n', '--threads', help='Number of threads', required=False, default=20)
args = vars(parser.parse_args())

# Extract arguments
variables = list(args['variables'])
ssps = args['ssps']
if ssps == "ssp":
    ssps = ["ssp126", "ssp245", "ssp585", "ssp370"]
else:
    ssps = ["historical"]

output_path = args['output_path']
path = r'./WGETs/'
name = args["wget"]
wget_variable = args["wget_var"]
ssp = args["wget_ssp"]
# NR modified as there is no requirement for wap
variables = ['ta', 'hus', 'ua', 'va']

# Scrape links
links = Scrape_WGET(path + name, wget_variable, ssp, startdate=1950, enddate=2100, variables=variables, ssps=ssps)
links = pd.melt(links, id_vars=['variables', 'ssps'])
links['value'] = links['value'].apply(lambda x: x.strip("'"))
print(links['value'])

# Download list function
def download_list(downloader, links, output_path):
    outpath, size, downloading = [], [], []
    for index, link in links.iterrows():
        file_path = os.path.join(output_path, os.path.basename(link['value']))
        try:
            if os.path.exists(file_path):
                print(f"File already exists: {file_path}")
                outpath.append(file_path)
                size.append(os.path.getsize(file_path))
                downloading.append(False)
            else:
                print(f"Downloading: {link['value']}")
                downloaded_file, file_size = downloader.get_file(link['value'], file_path, link_size=None)
                outpath.append(downloaded_file)
                size.append(file_size)
                downloading.append(True)
        except Exception as e:
            print(f"Failed to download {link['value']}: {e}")
            outpath.append(None)
            size.append(0)
            downloading.append(False)
    return outpath, size, downloading

# Initialize downloader
downloader = FileDownloader(max_threads=int(args["threads"]))

# Download files and collect outputs
down_start = time.time()
outpath, size, downloading = download_list(downloader, links, output_path)
down_end = time.time()

# Add metadata to DataFrame
links['output_path'] = outpath
links['size'] = size
links['download_executed'] = downloading
links['path_exists'] = links['output_path'].apply(lambda p: os.path.exists(p) if isinstance(p, str) else False)
links['download_time'] = down_end - down_start
links['download_size (GB)'] = np.sum(size) / 1e9
links['download_speed (GB/s)'] = (np.sum(size) / 1e9) / (down_end - down_start)

# Output logs
if not os.path.exists('./logs/' + name.strip('.sh')):
    os.makedirs('./logs/' + name.strip('.sh'))

links.to_csv('./logs/' + name.strip('.sh') + '.csv', index=False)

# Print summary
print('Download Time:', down_end - down_start)
print('Downloaded:', len(size), 'Files, with a size of:', np.sum(size) / 1e9, 'GB')
print((np.sum(size) / 1e9) / (down_end - down_start), 'GB/s')
